...
class UserController extends CommonController
{
	...
	public function changeStatus()
	{
		$id = I('post.id');
		$status = I('post.status');
		$lastId = M('user')->where(['id'=>$id])->setField('status', $status);
		if ($lastId) {
			$this->ajaxreturn(1);
		} else {
			$this->ajaxreturn(2);
	...
	public function upUser()
	 {
	 	//获取到修改的用户 id
		$id = I('get.id');
		$list = M('user')->field('id,username,status')->where(['id'=>$id])->find();
		//输出
		$this->assign('list', $list);

		$this->assign('session', session('adminInfo'));
		//指定跳转到的页面
		$this->display('User/upuser');
	}
	...
	{
		$id = I('post.id');
		$lastId = M('user')->where(['id'=>$id])->delete();
		if ($lastId) {
			$this->ajaxreturn(1);
		} else {
			$this->ajaxreturn(2);
